...
import com.octo_tte.R;
import com.octo_tte.ValidatedTicketView;
...
//                Log.e("Check", metadataText.toString());
...
//        TextView contentsTextView = (TextView) findViewById(R.id.contents_text_view);
        ...
        String secret_data = displayContents.subSequence(0,6).toString();
        Log.e("Data",secret_data);
        ArrayList<SecretCode> secretCodeList = secretCodeDatabase.getData();
        boolean check = false;
        for(SecretCode secretCode:secretCodeList){
            Log.e("Database",secretCode.getSecretcode());
            if(secretCode.getSecretcode().equals(secret_data))
                check = true;
        }
        Intent intent = new Intent(getApplicationContext(), ValidatedTicketView.class);
        if(check) {
//            contentsTextView.setText("Valid Ticket");
            intent.putExtra("validity", "valid");
            intent.putExtra("code", displayContents.toString());
            startActivity(intent);
        }
        else {
//            contentsTextView.setText("Invalid Ticcet");
            intent.putExtra("validity","invalid");
            intent.putExtra("code",displayContents.toString());
            startActivity(intent);
        }
        Log.e("Check", displayContents.toString());
